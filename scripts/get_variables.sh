...
HAVE_HCP=$(which hcp |grep "/opt/homebrew/bin/hcp" >/dev/null; echo $?)
...
HCP_CLIENT_ID=$(cat ~/code/hcp-client-id.txt)
#HCP_CLIENT_SECRET=$(op read op://private/hcp-cloud/client-secret)
HCP_CLIENT_SECRET=$(cat ~/code/hcp-client-secret.txt)
...
WORKSPACE_ID=$(curl -s \
  --header "Authorization: Bearer $TOKEN" \
  --header "Content-Type: application/vnd.api+json" \
  https://app.terraform.io/api/v2/organizations/${ORGANIZATION}/workspaces |jq -r '.data[] | select(.attributes.name == "2_hcp-clusters").id')
...
# Get the Vault Token:
VAULT_CLUSTER_NAME=1001-vault-cluster
...
export NOMAD_WORKSPACE_ID=$(curl -s \
  --header "Authorization: Bearer $TOKEN" \
  --header "Content-Type: application/vnd.api+json" \
  https://app.terraform.io/api/v2/organizations/${ORGANIZATION}/workspaces |jq -r '.data[] | select(.attributes.name == "5_nomad-cluster").id')
